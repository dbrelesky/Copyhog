...
| SHELL-03 | Global hotkey Shift+Cmd+C | Fixed: contains-based modifier check, DispatchQueue.main.async, NSApp.activate | Hotkey changed to Shift+Cmd+C, toggle fixed | `[x]` Complete | **satisfied** |
| SHELL-04 | Launch at login, silent | SMAppService.mainApp.register() + LSUIElement=true | 01-01: SMAppService implemented | `[x]` Complete | **satisfied** |
...
- Global hotkey toggle still uses private KVC key `statusItem` with cached fallback — fragile, may break across macOS versions
...
21 of 21 requirements are satisfied. SHELL-03 was fixed by changing the hotkey to Shift+Cmd+C with reliable modifier detection, proper threading via DispatchQueue.main.async, and NSApp.activate for cross-app toggle. SHELL-04 was verified — SMAppService.mainApp.register() and LSUIElement=true provide launch-at-login and silent background operation. All 4 E2E flows complete. The milestone passes with no functional gaps.
